...
Admission is Bun 1.4.2 from `.bun-version` and `packageManager`. The supported states are:
...
presentation, if admitted later, uses Shallot's public `captureFrame` contract. No GPU population
or frame claim is manufactured by this migration; missing seats are inconclusive.
