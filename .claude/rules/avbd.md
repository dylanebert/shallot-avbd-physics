...
GPU is wrong until ruled out: the oracle is not the suspect. Never lower it to f32. Physics changes run `bun test ./tests/*.oracle.ts`; real-device correctness/compile/perf use gym `pile`, hull kernels use `sat`. `headless.test.ts` is by-path build/lifecycle evidence, never solver parity; `testing.md` owns device-execution exceptions.
